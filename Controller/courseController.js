...
const courseValidationRules = [
  ...
  body("courseType")
    .isIn(["80% complete", "all open", "time to time"])
    .withMessage("Invalid type selected"),
  body("percentage")
    .if(body("courseType").equals("80% complete"))
    .isNumeric()
    .withMessage("Percentage must be a number for 80% complete type"),
  body("startTime")
    .if(body("courseType").equals("time to time"))
    .isLength({ min: 1 })
    .withMessage("Start time must be specified for time to time type"),
  body("endTime")
    .if(body("courseType").equals("time to time"))
    .isLength({ min: 1 })
    .withMessage("End time must be specified for time to time type"),
];
...
const createCourse = async (req, res) => {
  try {
    ...
    const course = new Course({
      ...
      dprice,
      courseType,
      percentage: courseType === "80% complete" ? percentage : null,
      startTime: courseType === "time to time" ? startTime : null,
      endTime: courseType === "time to time" ? endTime : null,
      createdBy: userId,
    });
...
const updateCourse = async (req, res) => {
    ...
    try {
      ...
      if (courseType === "percentage") {
        percentage = req.body.percentage || percentage;
      }
      if (courseType === "time to time") {
        startTime = req.body.startTime || startTime;
        endTime = req.body.endTime || endTime;
      }
      // Save the updated video document
      const updatedCourse = await course.save();
      res.json({ message: "Course updated successfully", video: updatedCourse });
    } catch (err) {
